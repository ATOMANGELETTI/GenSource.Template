...
- [ ] Checked the top-level `.agents/` folders (`memory/`, `personas/`,
      `prompts/`, `data/`, `documents/`, `workflows/`, `plans/`,
      `commands/`, `mcp/`, `core/`, `logs/`, `assets/`, `scripts/`) for
      existing material worth referencing from the new skill.
...
- [ ] `node .agents/skills/create-skill-pro/scripts/validate-skill.mjs <path-to-SKILL.md>` passes.
